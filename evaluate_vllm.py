...
from task_contract import evaluate_answer, ground_truth
...
def main():
    ...
    with (out/f'{a.split}_predictions.jsonl').open('w') as stream:
        for i,row in enumerate(rows):
            ...
            record={'index':i,'task_id':row['task_id'],'prediction':text,'truth':ground_truth(row),
                    ...
                    'prompt_tokens':len(output.prompt_token_ids),'output_tokens':len(output.outputs[0].token_ids),
                    **evaluate_answer(text,row)}
            stream.write(json.dumps(record,ensure_ascii=False)+'\n');stream.flush()
            predictions.append(record)
            print(json.dumps({'progress':i+1,'total':len(rows),'task':row['task_id'],
                              'correct':record['exact_match'],'format':record['format_valid']}),flush=True)
    ...
    for task in ('all','A2','P1'):
        ...
        if group:
            summary['metrics'][task]={'n':len(group),'exact_match':sum(r['exact_match'] for r in group)/len(group),
